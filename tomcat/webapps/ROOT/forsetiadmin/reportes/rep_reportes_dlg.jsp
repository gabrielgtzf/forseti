...
<%
	String reps_reportes_dlg = (String)request.getAttribute("reps_reportes_dlg");
	if(reps_reportes_dlg == null)
	{
   	...
   	JReportesBind1Set rep = new JReportesBind1Set(request, JUtil.getSesionAdmin(request).getID_Usuario(), request.getParameter("REPID"), "SAF-1");
	rep.ConCat(true);
	rep.Open();
	
	String titulo =  JUtil.getSesionAdmin(request).getSesion("REPS_REPORTES").generarTitulo(JUtil.Msj("SAF","REPS_REPORTES","VISTA",request.getParameter("proceso"),3));
...
<form onSubmit="return enviarlo(this)" action="/servlet/SAFReportesDlg" method="post" enctype="application/x-www-form-urlencoded" name="reps_reportes_dlg" target="ventImp">
<div id="topbar"> 
...
%>
   ...
   <tr> 
     <td align="center" class="titChicoNar">No existe filtro para este reporte, Presiona 
...
%>
 ...
 <%
					...
					{ 
%>
				  <span class="titChicoNar"> - Desde</span><br>
<%
...
%>
				<select class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() %>">
					...
					<option value="11"<% if(mes == 11) { out.print(" selected"); } %>>noviembre</option>
					<option value="12"<% if(mes == 12) { out.print(" selected"); } %>>diciembre</option>
				  </select><span class="titChicoNar"> - Hasta</span>
<%
						}
...
<%
						...
						else if(set.getAbsRow(i).getSecDefault().equals("ano"))
						{
							Calendar fecha = GregorianCalendar.getInstance();
         					int ano = JUtil.obtAno(fecha);
%>
				  <input class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() %>" type="text" value="<%= ano %>" size="5" maxlength="4"><span class="titChicoNar"> - Hasta</span>
 <%
						}
						else
						{
%>
				  <input class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() %>" type="text" value="<%= set.getAbsRow(i).getSecDefault() %>" size="5" maxlength="12"><span class="titChicoNar"> - Hasta</span>
 <%
 						}
					}				
				}
...
<%
					...
					{ 
%>
				  <span class="titChicoNar"> - Desde</span><br><input name="<%= set.getAbsRow(i).getSecDataName() %>" type="text" id="<%= set.getAbsRow(i).getSecDataName() %>" size="12" maxlength="15" readonly="true" value="<%= JUtil.obtFechaTxt(fecha,fechasql1) %>">
				  <a href="javascript:NewCal('<%= set.getAbsRow(i).getSecDataName() %>','<%= fechasql2 %>',false)"><img src="../../imgfsi/calendario.gif" title="<%= JUtil.Msj("GLB","GLB","DLG","CALENDARIO") %>" border="0" align="absmiddle"></a><span class="titChicoNar"> - Hasta</span><br>
<%	
					}				
				}
				else if(set.getAbsRow(i).getBindDataType().equals("DECIMAL") || set.getAbsRow(i).getBindDataType().equals("MONEY"))
...
<%
						...
						{
%>
				  <input class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() %>" type="text" value="<%= set.getAbsRow(i).getSecDefault() %>" size="10" maxlength="15"><span class="titChicoNar"> - Hasta</span>
 <%
 						}
					}				
				}
...
<%
						...
						{
%>				  
				  <input class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() %>" type="text" value="<%= set.getAbsRow(i).getSecDefault() %>" size="35" maxlength="254"><span class="titChicoNar"> - Hasta</span>
<%
						}	
					}				
				}
...
%>
				  <span class="titChicoNar"> - Desde</span><br><input class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() %>" type="text" readonly="true" value="<% if(request.getParameter(set.getAbsRow(i).getSecDataName()) != null) { out.print(request.getParameter(set.getAbsRow(i).getSecDataName())); } else { out.print(fsi_val2); } %>" size="4" maxlength="254"><a href="javascript:abrirCatalogo('../../forsetiadmin/listasfiltro_dlg.jsp?formul=reps_reportes_dlg&lista=<%= set.getAbsRow(i).getSecDataName() %>&idreporte=<%= request.getParameter("REPID") %>&idcolumna=<%= set.getAbsRow(i).getID_Column() %>&destino=<%= set.getAbsRow(i).getSecDataName() + "_FSIDESC" %>',250,350)"><img src="../../imgfsi/catalogo.gif" title="<%= JUtil.Msj("GLB","GLB","DLG","CATALOGO") %>" align="absmiddle" width="20" height="20" border="0"></a>
				    <input class="cpoBco" name="<%= set.getAbsRow(i).getSecDataName() + "_FSIDESC" %>" type="text" readonly="true" value="<% if(request.getParameter(set.getAbsRow(i).getSecDataName()) != null) { out.print(request.getParameter(set.getAbsRow(i).getSecDataName() + "_FSIDESC")); } else { out.print(fsi_desc2); } %>" size="35" maxlength="254"><span class="titChicoNar"> - Hasta</span>
<%
						}
...
%>
   <tr>
	  ...
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			<td width="45%" class="titCuerpoNar">Opciones de Exportacion</td>
...
%>
 <tr>
  	<td>
          ...
          <tr> 
            <td colspan="5" align="center" class="titCuerpoNar">Este reporte se 
              puede graficar</td>
          </tr>
          <tr> 
