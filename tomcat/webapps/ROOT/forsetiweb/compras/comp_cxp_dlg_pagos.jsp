...
<%
	String comp_cxp_dlg = (String)request.getAttribute("comp_cxp_dlg");
	if(comp_cxp_dlg == null)
	{
	...
	String titulo =  JUtil.getSesion(request).getSesion("COMP_CXP").generarTitulo(JUtil.Msj("CEF","COMP_CXP","VISTA",request.getParameter("proceso"),3));

	JProveeCXPSetV2 smod = new JProveeCXPSetV2(request);
	smod.m_Where = "Clave = '" + JUtil.p(request.getParameter("id")) + "'";
	smod.Open();
	JProveeProveeMasSetV2 smas = new JProveeProveeMasSetV2(request);
	smas.m_Where = "ID_Tipo = 'PR' and ID_Clave = '" + smod.getAbsRow(0).getID_ClaveProvee() + "'";
	smas.Open();
	...
	{
		ant.m_Where = "ID_ClaveProvee = '" + smod.getAbsRow(0).getID_ClaveProvee() + "' and ID_TipoProvee = 'PR' and Saldo > '0.00' and ID_TipoCP = 'ANT' and Status = 'G'";
		ant.Open();
	} 
%>
...
<% 
if(request.getParameter("proceso").equals("PAGAR_CXP") || request.getParameter("proceso").equals("DEVOLVER_ANTICIPO"))
{
%>
function configurarPago()
{
	var cantidad = document.comp_cxp_dlg_pagos.cantidad.value;
	var tc = document.comp_cxp_dlg_pagos.tc.value;
	var total = redondear(cantidad * tc, 2);
	
	var refer = "../../forsetiweb/pagos_dlg.jsp?formul=comp_cxp_dlg_pagos&va_tipo=compras&va_proc=<%= ((request.getParameter("proceso").equals("PAGAR_CXP")) ? "retiro" : "deposito" )%>&va_total=" + total + "&va_ident=<%= JUtil.getSesion(request).getSesion("COMP_CXP").getEspecial() %>";
...
%>
	...
	{
		alert('<%= JUtil.Msj("CEF","COMP_CXP","DLG","MSJ-PROCERR2",3) %>');
		return false;
	} 
<%
}
...
<form onSubmit="return enviarlo(this)" action="/servlet/CEFCompCXPDlg" method="post" enctype="application/x-www-form-urlencoded" name="comp_cxp_dlg_pagos" target="_self">
<div id="topbar"> 
            ...
            <td align="right" class="clockCef"> 
              <%  if(JUtil.getSesion(request).getID_Mensaje() == 0 || request.getParameter("proceso").equals("CONSULTAR_CUENTA")) { %>
        			...
        			<input type="submit" name="aceptar" value="<%= JUtil.Msj("GLB","GLB","GLB","ACEPTAR") %>">
       				<%  } %>
        			<input type="button" name="cancelar" onClick="javascript:document.location.href='/servlet/CEFCompCXPCtrl';" value="<%= JUtil.Msj("GLB","GLB","GLB","CANCELAR") %>">
            </td>
          </tr>
        </table> 
      </td>
		...
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr> 
            <td width="10%" height="21">		
				...
				<input name="entidad" type="hidden" value="<%= JUtil.getSesion(request).getSesion("COMP_CXP").getEspecial() %>">
...
%>
              <%= JUtil.Msj("GLB","GLB","GLB","PROVEEDOR") %></td>
...
%>
          <tr>
            ...
            <td><input name="clave" type="text" id="clave" size="7" maxlength="10"> 
              <a href="javascript:abrirCatalogo('../../forsetiweb/listas_dlg.jsp?formul=comp_cxp_dlg_pagos&lista=clave&idcatalogo=9&nombre=CONCEPTOS&destino=clave_nombre',250,350)"><img src="../../imgfsi/catalogo.gif" title="<%= JUtil.Msj("GLB","GLB","DLG","CATALOGO") %>" align="absmiddle" width="20" height="20" border="0"></a> 
...
<%
	...
	{
%>
document.comp_cxp_dlg_pagos.clave.value = '<% if(request.getParameter("clave") != null) { out.print( request.getParameter("clave") ); } else { out.print(""); } %>'
document.comp_cxp_dlg_pagos.clave_nombre.value = '<% if(request.getParameter("clave_nombre") != null) { out.print( request.getParameter("clave_nombre") ); } else { out.print(""); } %>' 
<%
	}
%>
document.comp_cxp_dlg_pagos.fecha.value = '<% if(request.getParameter("fecha") != null) { out.print( request.getParameter("fecha") ); } else { out.print(JUtil.obtFechaTxt(new Date(), "dd/MMM/yyyy")); } %>'
document.comp_cxp_dlg_pagos.concepto.value = '<% if(request.getParameter("concepto") != null) { out.print( request.getParameter("concepto") ); } else { out.print(""); } %>'
document.comp_cxp_dlg_pagos.idmoneda.value = '<% if(request.getParameter("idmoneda") != null) { out.print( request.getParameter("idmoneda") ); } else { out.print( setMon.getAbsRow(0).getClave() ); } %>'
document.comp_cxp_dlg_pagos.tc.value = '<% if(request.getParameter("tc") != null) { out.print( request.getParameter("tc") ); } else { out.print( setMon.getAbsRow(0).getTC() ); } %>'
document.comp_cxp_dlg_pagos.cantidad.value = '<% if(request.getParameter("cantidad") != null) { out.print( request.getParameter("cantidad") ); } else { out.print( smod.getAbsRow(0).getSaldo() ); } %>'
</script>
</body>
</html>
