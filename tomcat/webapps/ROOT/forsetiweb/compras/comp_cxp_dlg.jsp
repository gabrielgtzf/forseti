...
<%
	...
	smod.m_Where = "ID_Tipo = 'PR' and Clave = '" + JUtil.p(request.getParameter("id")) + "'";
	smod.Open();
	JProveeProveeMasSetV2 smas = new JProveeProveeMasSetV2(request);
	smas.m_Where = "ID_Tipo = 'PR' and ID_Clave = '" + JUtil.p(request.getParameter("id")) + "'";
	smas.Open();
...
function configurarPago()
{
	...
	var cantidad = document.comp_cxp_dlg.cantidad.value;
	var tc = document.comp_cxp_dlg.tc.value;
	var total = redondear(cantidad * tc, 2);
	
	var refer = "../../forsetiweb/pagos_dlg.jsp?formul=comp_cxp_dlg&va_tipo=compras&va_proc=retiro&va_total=" + total + "&va_ident=<%= JUtil.getSesion(request).getSesion("COMP_PROVEE").getEspecial() %>";
...
<form onSubmit="return enviarlo(this)" action="/servlet/CEFCompProveeDlg" method="post" enctype="application/x-www-form-urlencoded" name="comp_cxp_dlg" target="_self">
<div id="topbar"> 
            ...
            <td align="right" class="clockCef"> 
              <%  if(JUtil.getSesion(request).getID_Mensaje() == 0 || request.getParameter("proceso").equals("CONSULTAR_PROVEEDOR")) { %>
        			...
        			<input type="submit" name="aceptar" value="<%= JUtil.Msj("GLB","GLB","GLB","ACEPTAR") %>">
       				<%  } %>
        			<input type="button" name="cancelar" onClick="javascript:document.location.href='/servlet/<%= ( request.getParameter("proceso").equals("AGREGAR_ANTICIPO") || request.getParameter("proceso").equals("AGREGAR_CUENTA") ? "CEFCompProveeCtrl" : "CEFCompCXPCtrl" ) %>';" value="<%= JUtil.Msj("GLB","GLB","GLB","CANCELAR") %>">
            </td>
          </tr>
        </table> 
      </td>
		...
		<table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr> 
            <td width="10%" height="21">		
				...
				<input name="entidad" type="hidden" value="<%= JUtil.getSesion(request).getSesion("COMP_PROVEE").getEspecial() %>">
...
%>
          <tr>
            ...
            <td><input name="clave" type="text" id="clave" size="7" maxlength="10"> 
              <a href="javascript:abrirCatalogo('../../forsetiweb/listas_dlg.jsp?formul=comp_cxp_dlg&lista=clave&idcatalogo=9&nombre=CONCEPTOS&destino=clave_nombre',250,350)"><img src="../../imgfsi/catalogo.gif" title="<%= JUtil.Msj("GLB","GLB","DLG","CATALOGO") %>" align="absmiddle" width="20" height="20" border="0"></a> 
...
<%
	...
	{
%>
document.comp_cxp_dlg.clave.value = '<% if(request.getParameter("clave") != null) { out.print( request.getParameter("clave") ); } else { out.print(""); } %>'
document.comp_cxp_dlg.clave_nombre.value = '<% if(request.getParameter("clave_nombre") != null) { out.print( request.getParameter("clave_nombre") ); } else { out.print(""); } %>' 
<%
	}
%>
document.comp_cxp_dlg.fecha.value = '<% if(request.getParameter("fecha") != null) { out.print( request.getParameter("fecha") ); } else { out.print(JUtil.obtFechaTxt(new Date(), "dd/MMM/yyyy")); } %>'
document.comp_cxp_dlg.concepto.value = '<% if(request.getParameter("concepto") != null) { out.print( request.getParameter("concepto") ); } else { out.print(""); } %>'
document.comp_cxp_dlg.tc.value = '<% if(request.getParameter("tc") != null) { out.print( request.getParameter("tc") ); } else { out.print("1"); } %>'
document.comp_cxp_dlg.cantidad.value = '<% if(request.getParameter("cantidad") != null) { out.print( request.getParameter("cantidad") ); } else { out.print(""); } %>'
</script>
</body>
</html>
