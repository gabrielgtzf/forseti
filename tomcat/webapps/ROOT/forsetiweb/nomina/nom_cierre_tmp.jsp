...
function ventEm()
{
	...
	ventana = window.open('../../forsetiweb/filtro_esp_por_mes.jsp?fsi_vista=CEFNomCierreDiarioCtrl', '', parametrs);
	ventana.focus();
}
</script>
</head>
...
oCMenu.makeMenu('top1','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","HOY") %>','/servlet/CEFNomCierreDiarioCtrl?tiempo=HOY','cuerpo')
oCMenu.makeMenu('top2','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","SEMANA") %>','/servlet/CEFNomCierreDiarioCtrl?tiempo=SEM','cuerpo')
<%
...
%>
		oCMenu.makeMenu('top<%=i+3%>','','&nbsp;<%=str%>','/servlet/CEFNomCierreDiarioCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
	}
%>
