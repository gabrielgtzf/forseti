...
function ventEm()
{
	...
	ventana = window.open('../../forsetiweb/filtro_esp_por_mes.jsp?fsi_vista=CEFAdmCFDCtrl', '', parametrs);
	ventana.focus();
}
</script>
</head>
...
<script>
<%
	if(JUtil.getSesion(request).getSesion("ADM_CFDI").getEspecial().equals("CFDI"))
	{
%>
oCMenu.makeMenu('top0','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","ESP",2) %>','javascript:ventEm()')
oCMenu.makeMenu('top1','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","HOY") %>','/servlet/CEFAdmCFDCtrl?tiempo=HOY','cuerpo')
oCMenu.makeMenu('top2','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","SEMANA") %>','/servlet/CEFAdmCFDCtrl?tiempo=SEM','cuerpo')
<%
...
%>
oCMenu.makeMenu('top<%=i+3%>','','&nbsp;<%=str%>','/servlet/CEFAdmCFDCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
		}
%>
oCMenu.construct()
