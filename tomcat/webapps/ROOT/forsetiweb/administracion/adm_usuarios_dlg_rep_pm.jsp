...
<%
	...
	JProcessSet setRep = new JProcessSet(request);
    setRep.setSQL("select distinct Tipo from TBL_REPORTS ");
 	setRep.Open();
...
<%			
	for (int i = 0; i < setRep.getNumRows(); i++) 
	{
%>
			  <table width="100%" border="0" cellpadding="5" cellspacing="5" bgcolor="#0099FF">
                 <tr>
					<td class="titChico" align="center"><%= JUtil.Msj("CEF","MENU","GLB",setRep.getAbsRow(i).getSTS("Col1")) %></td>
				  </tr>
			  </table>
<%
		JProcessSet setTip = new JProcessSet(request);
		String strTip = "select distinct SubTipo from TBL_REPORTS where Tipo = '" + setRep.getAbsRow(i).getSTS("Col1") + "'";
    	setTip.setSQL(strTip);
		setTip.Open();
		
		for(int j = 0; j < setTip.getNumRows(); j++)
		{
			
%>
			<table width="100%" border="0" cellpadding="5" cellspacing="5">
                 <tr>
					<td class="cpoBcoNg"><%= JUtil.Msj("CEF","MENU",setRep.getAbsRow(i).getSTS("Col1"),setTip.getAbsRow(j).getSTS("Col1")) %></td>
				 </tr>
			</table>	
<%
			JUsuariosSubmoduloReportesPerm prm = new JUsuariosSubmoduloReportesPerm(request);
			prm.m_Where = "ID_Usuario = '" + JUtil.p(request.getParameter("id")) + "' and Tipo = '" + JUtil.p(setRep.getAbsRow(i).getSTS("Col1")) + "' and SubTipo = '" + JUtil.p(setTip.getAbsRow(j).getSTS("Col1"))  + "'";
			prm.Open();
...
%>
			<table width="100%" border="0" cellpadding="5" cellspacing="5">
            	<tr>
					<td width="10%" align="center"><input type="checkbox" name="PER_REP_<%= prm.getAbsRow(k).getID_Report() %>"<% if(prm.getAbsRow(k).getPermitido()) { out.print(" checked"); } %>/></td>
					<td><%= prm.getAbsRow(k).getDescription() %></td>
				 </tr>
			</table>
<%
			}
