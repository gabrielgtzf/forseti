...
<script>
<%
if(JUtil.getSesion(request).getSesion("ALM_UTENSILIOS").getVista().equals("EXISTENCIAS"))
{
%>
oCMenu.level[0].width=20
oCMenu.makeMenu('top0','','&nbsp;*','/servlet/CEFAlmUtensiliosCtrl?tiempo=TODO','cuerpo')
<%
	int i = 1;
	for (char c = 'A'; c <= 'Z'; c++) 
	{
...
%>
oCMenu.makeMenu('top<%=i++%>','','&nbsp;<%=str%>','/servlet/CEFAlmUtensiliosCtrl?tiempo=<%= str %>','cuerpo')
<%
	}
} 
else //Movimientos
...
oCMenu.makeMenu('top1','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","HOY",1)%>','/servlet/CEFAlmUtensiliosCtrl?tiempo=HOY','cuerpo')
oCMenu.makeMenu('top2','','&nbsp;<%= JUtil.Msj("GLB","GLB","GLB","SEMANA",1)%>','/servlet/CEFAlmUtensiliosCtrl?tiempo=SEM','cuerpo')
<%
...
%>
		oCMenu.makeMenu('top<%=i+3%>','','&nbsp;<%=str%>','/servlet/CEFAlmUtensiliosCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
	}
}
%>
