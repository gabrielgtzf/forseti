...
function ventEm()
{
	...
	ventana = window.open('../../forsetiweb/filtro_esp_por_mes.jsp?fsi_vista=CEFAlmChFisCtrl', '', parametrs);
	ventana.focus();
}
</script>
</head>
...
%>
		oCMenu.makeMenu('top<%=i%>','','&nbsp;<%=str%>','/servlet/CEFAlmChFisCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
	}
%>
