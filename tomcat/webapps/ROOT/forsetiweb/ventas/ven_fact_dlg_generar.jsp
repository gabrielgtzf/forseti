...
<%
    ...
    JVenFactSes rec = (JVenFactSes)session.getAttribute("ven_fact_dlg");
	...
	bv.m_Where = "Tipo = '0' and ID_EntidadVenta = '" + entven + "'";
	bv.Open();
	cv.m_OrderBy = "Clave ASC";
	cv.m_Where = "Tipo = '1' and ID_EntidadVenta = '" + entven + "'";
	cv.Open();
%>
...
<% 
	if(rec.getForma_Pago().equals("contado") && request.getParameter("proceso").equals("FACTURAR_VENTA"))
	{
%>
function configurarPago()
{
	...
	var refer = "../../forsetiweb/pagos_mult_dlg.jsp?formul=ven_fact_dlg_generar&va_tipo=ventas&va_proc=deposito&va_total=" + total + "&va_ident=<%= entven %>&va_cantidad=" + cantidad + "&va_idmon=" + idmon;
...
<form onSubmit="return enviarlo(this)" action="/servlet/CEFVenFactDlg" method="post" enctype="application/x-www-form-urlencoded" name="ven_fact_dlg_generar" target="_self">
<div id="topbar"> 
...
<%
	...
	if(request.getParameter("proceso").equals("FACTURAR_VENTA")) 
		out.print(JUtil.Msj("GLB","GLB","GLB","FACTURA"));
	else if(request.getParameter("proceso").equals("PEDIR_VENTA")) 
		out.print(JUtil.Msj("GLB","GLB","GLB","PEDIDO"));
	else if(request.getParameter("proceso").equals("REMISIONAR_VENTA")) 
		out.print(JUtil.Msj("GLB","GLB","GLB","REMISION"));
%>
              </td>
