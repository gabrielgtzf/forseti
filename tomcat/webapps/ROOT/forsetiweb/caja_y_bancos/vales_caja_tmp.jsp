...
function ventEm()
{
	...
	ventana = window.open('../../forsetiweb/filtro_esp_por_mes.jsp?fsi_vista=CEFValesCajaCtrl', '', parametrs);
	ventana.focus();
}
</script>
</head>
...
*************************************/
oCMenu.makeMenu('top0','','&nbsp;***','/servlet/CEFValesCajaCtrl?tiempo=TODO','cuerpo')
oCMenu.makeMenu('top1','','&nbsp;Especial','javascript:ventEm()')
oCMenu.makeMenu('top2','','&nbsp;Hoy','/servlet/CEFValesCajaCtrl?tiempo=HOY','cuerpo')
oCMenu.makeMenu('top3','','&nbsp;Semana','/servlet/CEFValesCajaCtrl?tiempo=SEM','cuerpo')
<%
...
%>
		oCMenu.makeMenu('top<%=i+4%>','','&nbsp;<%=str%>','/servlet/CEFValesCajaCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
	}
%>
