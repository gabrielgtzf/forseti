...
function ventEm()
{
	...
	ventana = window.open('../../forsetiweb/filtro_esp_por_mes.jsp?fsi_vista=CEFMovBancariosCtrl', '', parametrs);
	ventana.focus();
}
</script>
</head>
...
oCMenu.makeMenu('top1','','&nbsp;Hoy','/servlet/CEFMovBancariosCtrl?tiempo=HOY','cuerpo')
oCMenu.makeMenu('top2','','&nbsp;Semana','/servlet/CEFMovBancariosCtrl?tiempo=SEM','cuerpo')
<%
...
%>
		oCMenu.makeMenu('top<%=i+3%>','','&nbsp;<%=str%>','/servlet/CEFMovBancariosCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
	}
%>
