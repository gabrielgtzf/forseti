...
function ventEm()
{
	...
	ventana = window.open('../../forsetiweb/filtro_esp_por_mes.jsp?fsi_vista=CEFCierresCajaCtrl', '', parametrs);
	ventana.focus();
}
</script>
</head>
...
oCMenu.makeMenu('top1','','&nbsp;Hoy','/servlet/CEFCierresCajaCtrl?tiempo=HOY','cuerpo')
oCMenu.makeMenu('top2','','&nbsp;Semana','/servlet/CEFCierresCajaCtrl?tiempo=SEM','cuerpo')
<%
...
%>
		oCMenu.makeMenu('top<%=i+3%>','','&nbsp;<%=str%>','/servlet/CEFCierresCajaCtrl?tiempo=MAS&mes=<%=set.getAbsRow(i).getMes()%>&ano=<%=set.getAbsRow(i).getAno()%>','cuerpo')
<%
	}
%>
