...
public class JNomCredSalDlg extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      if(request.getParameter("proceso") != null && !request.getParameter("proceso").equals(""))
      {
        ...
        else if(request.getParameter("proceso").equals("ELIMINAR_CREDSAL"))
        {
          // Revisa si tiene permisos
          if(!getSesion(request).getPermiso("NOM_CREDSAL_ELIMINAR"))
          {
        	  idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "NOM_CREDSAL_ELIMINAR");
              getSesion(request).setID_Mensaje(idmensaje, mensaje);
              RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"NOM_CREDSAL_ELIMINAR","NCRS||||",mensaje);
              irApag("/forsetiweb/caja_mensajes.jsp", request, response);
              return;
          }
    ...
    public void Eliminar(HttpServletRequest request, HttpServletResponse response)
    		throws ServletException, IOException
    {
    	String str = "select * from sp_nom_credito_salario_eliminar( '" + p(request.getParameter("id")) + "') as (err integer, res varchar, clave smallint)";
    	      
    	JRetFuncBas rfb = new JRetFuncBas();
    			
    	doCallStoredProcedure(request, response, str, rfb);
    	RDP("CEF",getSesion(request).getConBD(),(rfb.getIdmensaje() == 0 ? "OK" : (rfb.getIdmensaje() == 4 ? "AL" : "ER")),getSesion(request).getID_Usuario(), "NOM_CREDSAL_ELIMINAR", "NCRS|" + rfb.getClaveret() + "|||",rfb.getRes());
    	irApag("/forsetiweb/caja_mensajes.jsp", request, response);
