...
public class JNomCierreDiarioCtrl extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      String nom_cierre = "";
      request.setAttribute("nom_cierre", nom_cierre);
      ...
      if(getSesion(request).getEst("NOM_CIERRE") == false)
      {
          ...
          getSesion(request).EstablecerCEF(request, "nom_cierre.png", "NOM_CIERRE");
          getSesion(request).getSesion("NOM_CIERRE").setParametros(Entidad, Tiempo, "", set.getAbsRow(0).getDescripcion(), JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3), "");
          getSesion(request).getSesion("NOM_CIERRE").setOrden(p(request.getParameter("etq")),"");
          getSesion(request).getSesion("NOM_CIERRE").setEspecial(Integer.toString(set.getAbsRow(0).getID_Sucursal()));

          getSesion(request).setID_Mensaje(idmensaje, mensaje);
          RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"NOM_CIERRE","NCIE||" + set.getAbsRow(0).getID_Sucursal() + "||","");
          irApag("/forsetiweb/nomina/nom_cierre_vsta.jsp",request,response);
          return;
      }
      if(request.getParameter("entidad") != null && !request.getParameter("entidad").equals(""))
      {
          ...
          {
        	  String Entidad = "ID_Sucursal = '" + set.getAbsRow(0).getID_Sucursal() + "'";
        	  getSesion(request).getSesion("NOM_CIERRE").setEntidad(Entidad,set.getAbsRow(0).getDescripcion());
        	  getSesion(request).getSesion("NOM_CIERRE").setEspecial(Integer.toString(set.getAbsRow(0).getID_Sucursal()));
          }
          else
          {
        	  idmensaje = 1; mensaje += JUtil.Msj("GLB", "GLB", "GLB", "ERROR-PARAM", 1); // 1 Error de entidad
        	  RDP("CEF",getSesion(request).getConBD(),"IA",getSesion(request).getID_Usuario(),"NOM_CIERRE","NCIE||" + p(request.getParameter("entidad")) + "||",mensaje);
          }
      ...
      else if(request.getParameter("tiempo") != null && !request.getParameter("tiempo").equals(""))
      {
	  	  if(request.getParameter("tiempo").equals("HOY"))
	  	  {
	  		  Calendar fecha = GregorianCalendar.getInstance();
	  		  String Tiempo = "date_part('day',ID_FechaMovimiento) = " + JUtil.obtDia(fecha) + "  AND date_part('month',ID_FechaMovimiento) = " + JUtil.obtMes(fecha) + " AND date_part('year',ID_FechaMovimiento) = " + JUtil.obtAno(fecha);
	  		  getSesion(request).getSesion("NOM_CIERRE").setTiempo(Tiempo, JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3));
	  	  }
	  	  else if(request.getParameter("tiempo").equals("SEM"))
	  	  {
	  		  ...
	  		  getSesion(request).getSesion("NOM_CIERRE").setTiempo(Tiempo,JUtil.Msj("GLB", "GLB", "GLB", "SEMANA", 3));
	  	  }
	  	  else if(request.getParameter("tiempo").equals("MAS"))
	  	  {
	  		  if(request.getParameter("mes") != null && request.getParameter("ano") != null && !request.getParameter("mes").equals("") && !request.getParameter("ano").equals(""))
	  		  {
	  			  ...
	  			  getSesion(request).getSesion("NOM_CIERRE").setTiempo(Tiempo,TiempoTit);
	  		  }
	  		  else
	  		  {
      ...
      {
    	  getSesion(request).getSesion("NOM_CIERRE").setOrden(p(request.getParameter("etq")),p(request.getParameter("orden")));
      }
