...
public class JVentasCotizacionesSet extends JManejadorSet
{
	public JVentasCotizacionesSet(HttpServletRequest request)
	{
		m_Select = " * FROM view_ventas_cotizaciones_modulo ";
		m_PageSize = 50;
		this.request = request;
	}

	public view_ventas_cotizaciones_modulo getRow(int row)
	{
		return (view_ventas_cotizaciones_modulo)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_ventas_cotizaciones_modulo getAbsRow(int row)
	{
		return (view_ventas_cotizaciones_modulo)m_Rows.elementAt(row);
	}
  ...
  protected void BindRow()
	{
		try
		{
			view_ventas_cotizaciones_modulo pNode = new view_ventas_cotizaciones_modulo();
			...
			pNode.setID_Cotizacion(m_RS.getLong("ID_Cotizacion"));
