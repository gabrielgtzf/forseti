...
public class JVentasDevolucionesSet extends JManejadorSet
{
	public JVentasDevolucionesSet(HttpServletRequest request)
	{
		m_Select = " * FROM view_ventas_devoluciones_modulo ";
		m_PageSize = 50;
		this.request = request;
	}

	public view_ventas_devoluciones_modulo getRow(int row)
	{
		return (view_ventas_devoluciones_modulo)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_ventas_devoluciones_modulo getAbsRow(int row)
	{
		return (view_ventas_devoluciones_modulo)m_Rows.elementAt(row);
	}
  ...
  protected void BindRow()
	{
		try
		{
			view_ventas_devoluciones_modulo pNode = new view_ventas_devoluciones_modulo();
