...
public class JVentasFactSetDetV2 extends JManejadorSet
{
	public JVentasFactSetDetV2(HttpServletRequest request, String tipomov)
	{
		if(tipomov.equals("FACTURAS"))
			m_Select = " * FROM view_ventas_facturas_det ";
		else if(tipomov.equals("PEDIDOS"))
			m_Select = " * FROM view_ventas_pedidos_det ";
		else if(tipomov.equals("DEVOLUCIONES"))
			m_Select = " * FROM view_ventas_devoluciones_det ";
		else if(tipomov.equals("AGR_DEVOL"))
			m_Select = " * FROM view_ventas_agr_devol_det ";
		else if(tipomov.equals("COTIZACIONES"))
			m_Select = " * FROM view_ventas_cotizaciones_det ";
		else if(tipomov.equals("REMISIONES"))
			m_Select = " * FROM view_ventas_remisiones_det ";
	...
	public view_ventas_facturas_det getRow(int row)
	{
		return (view_ventas_facturas_det)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_ventas_facturas_det getAbsRow(int row)
	{
		return (view_ventas_facturas_det)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			view_ventas_facturas_det pNode = new view_ventas_facturas_det();
