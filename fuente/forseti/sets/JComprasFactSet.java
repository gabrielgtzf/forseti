...
public class JComprasFactSet extends JManejadorSet
{
	public JComprasFactSet(HttpServletRequest request)
	{
		m_Select = " * FROM view_compras_facturas_modulo ";
		m_PageSize = 50;
		this.request = request;
	}

	public view_compras_facturas_modulo getRow(int row)
	{
		return (view_compras_facturas_modulo)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_compras_facturas_modulo getAbsRow(int row)
	{
		return (view_compras_facturas_modulo)m_Rows.elementAt(row);
	}
  ...
  protected void BindRow()
	{
		try
		{
			view_compras_facturas_modulo pNode = new view_compras_facturas_modulo();
