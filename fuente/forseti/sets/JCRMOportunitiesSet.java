...
public class JCRMOportunitiesSet extends JManejadorSet
{
	public JCRMOportunitiesSet(HttpServletRequest request)
	{
		m_Select = " * FROM tbl_crmk_oportunities";
		m_PageSize = 50;
		this.request = request;
	}

	public tbl_crmk_oportunities getRow(int row)
	{
		return (tbl_crmk_oportunities)m_Rows.elementAt((getFloorRow() + row));
	}

	public tbl_crmk_oportunities getAbsRow(int row)
	{
		return (tbl_crmk_oportunities)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			tbl_crmk_oportunities pNode = new tbl_crmk_oportunities();
