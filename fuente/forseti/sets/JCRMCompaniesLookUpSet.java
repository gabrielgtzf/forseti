...
public class JCRMCompaniesLookUpSet extends JManejadorSet
{
	public JCRMCompaniesLookUpSet(HttpServletRequest request)
	{
		m_Select = " * FROM tbl_crmk_companies_lookup";
		m_PageSize = 50;
		this.request = request;
	}

	public tbl_crmk_companies_lookup getRow(int row)
	{
		return (tbl_crmk_companies_lookup)m_Rows.elementAt((getFloorRow() + row));
	}

	public tbl_crmk_companies_lookup getAbsRow(int row)
	{
		return (tbl_crmk_companies_lookup)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			tbl_crmk_companies_lookup pNode = new tbl_crmk_companies_lookup();
