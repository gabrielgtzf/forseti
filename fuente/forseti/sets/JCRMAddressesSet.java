...
public class JCRMAddressesSet extends JManejadorSet
{
	public JCRMAddressesSet(HttpServletRequest request)
	{
		m_Select = " * FROM tbl_crmk_addresses";
		m_PageSize = 50;
		this.request = request;
	}

	public tbl_crmk_addresses getRow(int row)
	{
		return (tbl_crmk_addresses)m_Rows.elementAt((getFloorRow() + row));
	}

	public tbl_crmk_addresses getAbsRow(int row)
	{
		return (tbl_crmk_addresses)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			tbl_crmk_addresses pNode = new tbl_crmk_addresses();
