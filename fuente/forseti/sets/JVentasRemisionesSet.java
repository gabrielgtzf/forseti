...
public class JVentasRemisionesSet extends JManejadorSet
{
	public JVentasRemisionesSet(HttpServletRequest request)
	{
		m_Select = " * FROM view_ventas_remisiones_modulo ";
		m_PageSize = 50;
		this.request = request;
	}

	public view_ventas_remisiones_modulo getRow(int row)
	{
		return (view_ventas_remisiones_modulo)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_ventas_remisiones_modulo getAbsRow(int row)
	{
		return (view_ventas_remisiones_modulo)m_Rows.elementAt(row);
	}
  ...
  protected void BindRow()
	{
		try
		{
			view_ventas_remisiones_modulo pNode = new view_ventas_remisiones_modulo();
			...
			pNode.setID_Remision(m_RS.getLong("ID_Remision"));
