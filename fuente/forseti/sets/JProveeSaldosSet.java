...
public class JProveeSaldosSet extends JManejadorSet
{
	public JProveeSaldosSet(HttpServletRequest request)
	{
		m_Select = " * FROM view_provee_saldos";
		m_PageSize = 50;
		this.request = request;
	}

	public view_provee_saldos getRow(int row)
	{
		return (view_provee_saldos)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_provee_saldos getAbsRow(int row)
	{
		return (view_provee_saldos)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			view_provee_saldos pNode = new view_provee_saldos();
