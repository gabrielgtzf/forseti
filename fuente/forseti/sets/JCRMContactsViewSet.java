...
public class JCRMContactsViewSet extends JManejadorSet
{
	public JCRMContactsViewSet(HttpServletRequest request)
	{
		m_Select = " * FROM view_crm_contacts";
		m_PageSize = 50;
		this.request = request;
	}

	public view_crm_contacts getRow(int row)
	{
		return (view_crm_contacts)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_crm_contacts getAbsRow(int row)
	{
		return (view_crm_contacts)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			view_crm_contacts pNode = new view_crm_contacts();
