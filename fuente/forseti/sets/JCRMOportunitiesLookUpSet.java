...
public class JCRMOportunitiesLookUpSet extends JManejadorSet
{
	public JCRMOportunitiesLookUpSet(HttpServletRequest request)
	{
		m_Select = " * FROM tbl_crmk_oportunities_lookup";
		m_PageSize = 50;
		this.request = request;
	}

	public tbl_crmk_oportunities_lookup getRow(int row)
	{
		return (tbl_crmk_oportunities_lookup)m_Rows.elementAt((getFloorRow() + row));
	}

	public tbl_crmk_oportunities_lookup getAbsRow(int row)
	{
		return (tbl_crmk_oportunities_lookup)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			tbl_crmk_oportunities_lookup pNode = new tbl_crmk_oportunities_lookup();
			...
			pNode.setTP_lookup(m_RS.getString("TP_lookup"));
