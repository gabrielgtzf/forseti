...
public class JPACServidoresSet extends JManejadorSet
{
	public JPACServidoresSet(HttpServletRequest request)
	{
		m_Select = " * FROM tbl_pac_servidores";
		m_PageSize = 50;
		this.request = request;
	}

	public tbl_pac_servidores getRow(int row)
	{
		return (tbl_pac_servidores)m_Rows.elementAt((getFloorRow() + row));
	}

	public tbl_pac_servidores getAbsRow(int row)
	{
		return (tbl_pac_servidores)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			tbl_pac_servidores pNode = new tbl_pac_servidores();
