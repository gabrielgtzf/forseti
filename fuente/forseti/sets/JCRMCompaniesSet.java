...
public class JCRMCompaniesSet extends JManejadorSet
{
	public JCRMCompaniesSet(HttpServletRequest request)
	{
		m_Select = " * FROM tbl_crmk_companies";
		m_PageSize = 50;
		this.request = request;
	}

	public tbl_crmk_companies getRow(int row)
	{
		return (tbl_crmk_companies)m_Rows.elementAt((getFloorRow() + row));
	}

	public tbl_crmk_companies getAbsRow(int row)
	{
		return (tbl_crmk_companies)m_Rows.elementAt(row);
	}
	...
	protected void BindRow()
	{
		try
		{
			tbl_crmk_companies pNode = new tbl_crmk_companies();
