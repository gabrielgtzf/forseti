...
public class JClientClientMasSetV2 extends JManejadorSet
{
	public JClientClientMasSetV2(HttpServletRequest request)
	{
		m_Select = " * FROM view_client_client_mas ";
		m_PageSize = 50;
		this.request = request;
	}

	public view_client_client_mas getRow(int row)
	{
		return (view_client_client_mas)m_Rows.elementAt((getFloorRow() + row));
	}

	public view_client_client_mas getAbsRow(int row)
	{
		return (view_client_client_mas)m_Rows.elementAt(row);
	}
  ...
  protected void BindRow()
	{
		try
		{
			view_client_client_mas pNode = new view_client_client_mas();
