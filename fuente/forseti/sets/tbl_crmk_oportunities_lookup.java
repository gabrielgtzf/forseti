/*
    Forseti, El ERP Gratuito para PyMEs
    Copyright (C) 2015 Gabriel Gutiérrez Fuentes.

    This program is free software: you can redistribute it and/or modify
    it under the terms of the GNU Affero General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Affero General Public License for more details.

    You should have received a copy of the GNU Affero General Public License
    along with this program.  If not, see <http://www.gnu.org/licenses/>.
*/
package forseti.sets;

public class tbl_crmk_oportunities_lookup
{
	private String m_ID_Section;
	private int m_PG_lookup;
	private String m_VL_lookup;
	private String m_TP_lookup;
	private int m_BO_active;
	private String m_TX_comments;
	private String m_TR_es;
	private String m_TR_en;
	private String m_TR_de;
	private String m_TR_it;
	private String m_TR_fr;
	private String m_TR_pt;
	private String m_TR_ca;
	private String m_TR_eu;
	private String m_TR_ja;
	private String m_TR_cn;
	private String m_TR_tw;
	private String m_TR_fi;
	private String m_TR_ru;
	private String m_TR_nl;
	private String m_TR_th;
	private String m_TR_cs;
	private String m_TR_uk;
	private String m_TR_no;
	private String m_TR_ko;
	private String m_TR_sk;
	private String m_TR_pl;
	private String m_TR_vn;

	public void setID_Section(String ID_Section)
	{
		m_ID_Section = ID_Section;
	}

	public void setPG_lookup(int PG_lookup)
	{
		m_PG_lookup = PG_lookup;
	}

	public void setVL_lookup(String VL_lookup)
	{
		m_VL_lookup = VL_lookup;
	}

	public void setTP_lookup(String TP_lookup)
	{
		m_TP_lookup = TP_lookup;
	}

	public void setBO_active(int BO_active)
	{
		m_BO_active = BO_active;
	}

	public void setTX_comments(String TX_comments)
	{
		m_TX_comments = TX_comments;
	}

	public void setTR_es(String TR_es)
	{
		m_TR_es = TR_es;
	}

	public void setTR_en(String TR_en)
	{
		m_TR_en = TR_en;
	}

	public void setTR_de(String TR_de)
	{
		m_TR_de = TR_de;
	}

	public void setTR_it(String TR_it)
	{
		m_TR_it = TR_it;
	}

	public void setTR_fr(String TR_fr)
	{
		m_TR_fr = TR_fr;
	}

	public void setTR_pt(String TR_pt)
	{
		m_TR_pt = TR_pt;
	}

	public void setTR_ca(String TR_ca)
	{
		m_TR_ca = TR_ca;
	}

	public void setTR_eu(String TR_eu)
	{
		m_TR_eu = TR_eu;
	}

	public void setTR_ja(String TR_ja)
	{
		m_TR_ja = TR_ja;
	}

	public void setTR_cn(String TR_cn)
	{
		m_TR_cn = TR_cn;
	}

	public void setTR_tw(String TR_tw)
	{
		m_TR_tw = TR_tw;
	}

	public void setTR_fi(String TR_fi)
	{
		m_TR_fi = TR_fi;
	}

	public void setTR_ru(String TR_ru)
	{
		m_TR_ru = TR_ru;
	}

	public void setTR_nl(String TR_nl)
	{
		m_TR_nl = TR_nl;
	}

	public void setTR_th(String TR_th)
	{
		m_TR_th = TR_th;
	}

	public void setTR_cs(String TR_cs)
	{
		m_TR_cs = TR_cs;
	}

	public void setTR_uk(String TR_uk)
	{
		m_TR_uk = TR_uk;
	}

	public void setTR_no(String TR_no)
	{
		m_TR_no = TR_no;
	}

	public void setTR_ko(String TR_ko)
	{
		m_TR_ko = TR_ko;
	}

	public void setTR_sk(String TR_sk)
	{
		m_TR_sk = TR_sk;
	}

	public void setTR_pl(String TR_pl)
	{
		m_TR_pl = TR_pl;
	}

	public void setTR_vn(String TR_vn)
	{
		m_TR_vn = TR_vn;
	}


	public String getID_Section()
	{
		return m_ID_Section;
	}

	public int getPG_lookup()
	{
		return m_PG_lookup;
	}

	public String getVL_lookup()
	{
		return m_VL_lookup;
	}

	public String getTP_lookup()
	{
		return m_TP_lookup;
	}

	public int getBO_active()
	{
		return m_BO_active;
	}

	public String getTX_comments()
	{
		return m_TX_comments;
	}

	public String getTR_es()
	{
		return m_TR_es;
	}

	public String getTR_en()
	{
		return m_TR_en;
	}

	public String getTR_de()
	{
		return m_TR_de;
	}

	public String getTR_it()
	{
		return m_TR_it;
	}

	public String getTR_fr()
	{
		return m_TR_fr;
	}

	public String getTR_pt()
	{
		return m_TR_pt;
	}

	public String getTR_ca()
	{
		return m_TR_ca;
	}

	public String getTR_eu()
	{
		return m_TR_eu;
	}

	public String getTR_ja()
	{
		return m_TR_ja;
	}

	public String getTR_cn()
	{
		return m_TR_cn;
	}

	public String getTR_tw()
	{
		return m_TR_tw;
	}

	public String getTR_fi()
	{
		return m_TR_fi;
	}

	public String getTR_ru()
	{
		return m_TR_ru;
	}

	public String getTR_nl()
	{
		return m_TR_nl;
	}

	public String getTR_th()
	{
		return m_TR_th;
	}

	public String getTR_cs()
	{
		return m_TR_cs;
	}

	public String getTR_uk()
	{
		return m_TR_uk;
	}

	public String getTR_no()
	{
		return m_TR_no;
	}

	public String getTR_ko()
	{
		return m_TR_ko;
	}

	public String getTR_sk()
	{
		return m_TR_sk;
	}

	public String getTR_pl()
	{
		return m_TR_pl;
	}

	public String getTR_vn()
	{
		return m_TR_vn;
	}


}

