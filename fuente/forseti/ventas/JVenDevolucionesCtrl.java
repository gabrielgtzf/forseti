...
public class JVenDevolucionesCtrl extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      String ven_dev = "";
      request.setAttribute("ven_dev",ven_dev);

      String mensaje = ""; short idmensaje = -1;
      String usuario = getSesion(request).getID_Usuario();

      if(!getSesion(request).getPermiso("VEN_DEV"))
      {
          idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "VEN_DEV");
          getSesion(request).setID_Mensaje(idmensaje, mensaje);
          RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"VEN_DEV","VDEV||||",mensaje);
          irApag("/forsetiweb/caja_mensajes_vsta.jsp", request, response);
          return;
      }

      // establece en la sesion que los mensajes se estan configurando por primera ocasion
      if(getSesion(request).getEst("VEN_DEV") == false)
      {
        ...
        getSesion(request).EstablecerCEF(request, "ven_dev.png", "VEN_DEV");
        getSesion(request).getSesion("VEN_DEV").setParametros(Entidad, Tiempo, "", set.getAbsRow(0).getDescripcion(), JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3), JUtil.Elm(JUtil.Msj("CEF", "VEN_DEV", "VISTA", "STATUS"),1));
        getSesion(request).getSesion("VEN_DEV").setOrden(p(request.getParameter("etq")),"");
        getSesion(request).getSesion("VEN_DEV").setEspecial(Integer.toString(set.getAbsRow(0).getID_Entidad()));
  	  
        getSesion(request).setID_Mensaje(idmensaje, mensaje);
        RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"VEN_DEV","VDEV||" + set.getAbsRow(0).getID_Entidad() + "||","");
        irApag("/forsetiweb/ventas/ven_dev_vsta.jsp",request,response);
        return;
      }
      ...
      if(request.getParameter("entidad") != null && !request.getParameter("entidad").equals(""))
      {
          ...
          {
          	String Entidad = "ID_Entidad = '" + set.getAbsRow(0).getID_Entidad() + "'";
          	getSesion(request).getSesion("VEN_DEV").setEntidad(Entidad,set.getAbsRow(0).getDescripcion());
          	getSesion(request).getSesion("VEN_DEV").setEspecial(Integer.toString(set.getAbsRow(0).getID_Entidad()));
          }
          else
          {
          	idmensaje = 1; mensaje += JUtil.Msj("GLB", "GLB", "GLB", "ERROR-PARAM", 1); // 1 Error de entidad
          	RDP("CEF",getSesion(request).getConBD(),"IA",getSesion(request).getID_Usuario(),"VEN_DEV","VDEV||" + p(request.getParameter("entidad")) + "||",mensaje);
          }
      }
      else if(request.getParameter("tiempo") != null && !request.getParameter("tiempo").equals(""))
      {
    	  if(request.getParameter("tiempo").equals("HOY"))
    	  {
    		  Calendar fecha = GregorianCalendar.getInstance();
    		  String Tiempo = "date_part('day',Fecha) = " + JUtil.obtDia(fecha) + "  AND date_part('month',Fecha) = " + JUtil.obtMes(fecha) + " AND date_part('year',Fecha) = " + JUtil.obtAno(fecha);
    		  getSesion(request).getSesion("VEN_DEV").setTiempo(Tiempo, JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("SEM"))
    	  {
    		  ...
    		  getSesion(request).getSesion("VEN_DEV").setTiempo(Tiempo,JUtil.Msj("GLB", "GLB", "GLB", "SEMANA", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("MAS"))
    	  {
    		  if(request.getParameter("mes") != null && request.getParameter("ano") != null && !request.getParameter("mes").equals("") && !request.getParameter("ano").equals(""))
    		  {
    			  ...
    			  getSesion(request).getSesion("VEN_DEV").setTiempo(Tiempo,TiempoTit);
    		  }
    		  else
    		  {
      ...
      else if(request.getParameter("status") != null && !request.getParameter("status").equals(""))
      {
    	  ...
    	  if(request.getParameter("status").equals("TODOS"))
    		  getSesion(request).getSesion("VEN_DEV").setStatus("",JUtil.Elm(sts,1));
    	  else if(request.getParameter("status").equals("GENERADOS"))
    		  getSesion(request).getSesion("VEN_DEV").setStatus("Status = 'G'",JUtil.Elm(sts,2));
    	  else if(request.getParameter("status").equals("CANCELADOS"))
    		  getSesion(request).getSesion("VEN_DEV").setStatus("Status = 'C'",JUtil.Elm(sts,3));
    	  else if(request.getParameter("status").equals("EMITIDOS"))
    		  getSesion(request).getSesion("VEN_DEV").setStatus("Status = 'E'",JUtil.Elm(sts,4));
    	  else if(request.getParameter("status").equals("REVERTIDOS"))
    		  getSesion(request).getSesion("VEN_DEV").setStatus("Status = 'R'",JUtil.Elm(sts,5));
    	  else
    	  {
      ...
      {
       	  getSesion(request).getSesion("VEN_DEV").setOrden(p(request.getParameter("etq")),p(request.getParameter("orden")));
      }
      getSesion(request).setID_Mensaje(idmensaje, mensaje);
      irApag("/forsetiweb/ventas/ven_dev_vsta.jsp", request, response);
