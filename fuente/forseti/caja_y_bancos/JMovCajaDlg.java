...
public class JMovCajaDlg extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      {
    	JBancosSetIdsV2 setids = new JBancosSetIdsV2(request,getSesion(request).getID_Usuario(),"1",getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial());
        setids.Open();   
        ...
        if(!request.getParameter("proceso").equals("AGREGAR_DEPOSITO") && !request.getParameter("proceso").equals("AGREGAR_RETIRO") && !request.getParameter("proceso").equals("TRASPASAR_FONDO") && request.getParameter("ID") != null)
        {
        	...
        	set.m_Where = "Tipo = '1' and Clave = '" + setids.getAbsRow(0).getID() + "' and ID = '" + p(request.getParameter("ID")) + "'";
        	set.Open();
        	if(set.getNumRows() < 1)
        	{
        		idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "BANCAJ_CAJAS");
        		getSesion(request).setID_Mensaje(idmensaje, mensaje);
        		RDP("CEF",getSesion(request).getConBD(),"AL",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS","MCAJ|" + request.getParameter("ID") + "|" + setids.getAbsRow(0).getID() + "||",mensaje);
        		irApag("/forsetiweb/caja_mensajes.jsp", request, response);
        		return;
        	}
        }
        ...
        if(request.getParameter("proceso").equals("AGREGAR_DEPOSITO") || request.getParameter("proceso").equals("AGREGAR_RETIRO"))
        {
          ...
          if(!getSesion(request).getPermiso("BANCAJ_CAJAS_AGREGAR"))
          {
            idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "BANCAJ_CAJAS_AGREGAR");
            getSesion(request).setID_Mensaje(idmensaje, mensaje);
            RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS_AGREGAR","MCAJ||||",mensaje);
            irApag("/forsetiweb/caja_mensajes.jsp", request, response);
            return;
          }
          ...
          else // Como el subproceso no es ENVIAR ni AGR_PART ni EDIT_PART ni BORR_PART, abre la ventana del proceso de AGREGADO para agregar `por primera vez
          {
            ...
            JMovBancariosSes pol = (JMovBancariosSes)ses.getAttribute("mov_cajas_dlg");
            if(pol == null)
            {
              pol = new JMovBancariosSes(request, getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial(), getSesion(request).getID_Usuario(), "CAJAS", false, (request.getParameter("proceso").equals("AGREGAR_DEPOSITO") ? "deposito" : "retiro"));
       	      ses.setAttribute("mov_cajas_dlg", pol);
            }
            else
              pol.resetear(request, getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial(), getSesion(request).getID_Usuario(), "CAJAS", false, (request.getParameter("proceso").equals("AGREGAR_DEPOSITO") ? "deposito" : "retiro"));
        ...
        {
          // Revisa si tiene permisos
          if(!getSesion(request).getPermiso("BANCAJ_CAJAS_TRASPASAR"))
          {
            idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "BANCAJ_CAJAS_TRASPASAR");
            getSesion(request).setID_Mensaje(idmensaje, mensaje);
            RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS_TRASPASAR","MCAJ||||",mensaje);
            irApag("/forsetiweb/caja_mensajes.jsp", request, response);
            return;
          }
          ...
          else // Como el subproceso no es ENVIAR
          {
            ...
            if(pol == null)
            {
              pol = new JMovBancariosSes(request, getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial(), getSesion(request).getID_Usuario(), "CAJAS", true, "retiro");
       	      ses.setAttribute("mov_cajas_dlg", pol);
            }
            else
              pol.resetear(request, getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial(), getSesion(request).getID_Usuario(), "CAJAS", true, "retiro");
        ...
        {
        	if(!getSesion(request).getPermiso("BANCAJ_CAJAS_CONSULTAR"))
            {
              idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "BANCAJ_CAJAS_CONSULTAR");
              getSesion(request).setID_Mensaje(idmensaje, mensaje);
              RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS_CONSULTAR","MCAJ||||",mensaje);
              irApag("/forsetiweb/caja_mensajes.jsp", request, response);
              return;
            }
            ...
            if(request.getParameter("ID") != null)
            {
              ...
              if (valoresParam.length == 1)
              {
                  ...
                  JRastreo rastreo = new JRastreo(	request, getSesion(request).getSesion("BANCAJ_CAJAS").generarTitulo(JUtil.Msj("CEF","BANCAJ_CAJAS","VISTA","CONSULTAR_MOVIMIENTO",3)),
                  								"MCAJ",request.getParameter("ID"));
                  ...
                  RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS_CONSULTAR","MCAJ|" + request.getParameter("ID") + "|" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "||","");
        ...
        else if(request.getParameter("proceso").equals("CONSULTAR_MOVIMIENTO"))
        {
          ...
          if(request.getParameter("ID") != null)
          {
            ...
            if(valoresParam.length == 1)
            {
                ...
                if(pol == null)
                {
                	pol = new JMovBancariosSes(request, getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial(), getSesion(request).getID_Usuario(), "CAJAS", false, "");
                	ses.setAttribute("mov_cajas_dlg", pol);
                }
                else
                	pol.resetear(request, getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial(), getSesion(request).getID_Usuario(), "CAJAS", false, "");
                ...
                RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS","MCAJ|" + request.getParameter("ID") + "|" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "||","");
                getSesion(request).setID_Mensaje(idmensaje, mensaje);
                irApag("/forsetiweb/caja_y_bancos/mov_bancaj_dlg.jsp", request, response);
                return;
            }
        ...
        {
          // Revisa si tiene permisos
          if(!getSesion(request).getPermiso("BANCAJ_CAJAS_CANCELAR"))
          {
        	  idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "BANCAJ_CAJAS_CANCELAR");
        	  getSesion(request).setID_Mensaje(idmensaje, mensaje);
        	  RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS_CANCELAR","MCAJ||||",mensaje);
        	  irApag("/forsetiweb/caja_mensajes.jsp", request, response);
        	  return;
          }
        ...
        {
          // Revisa si tiene permisos
          if(!getSesion(request).getPermiso("BANCAJ_CAJAS_GENPROC"))
          {
        	  idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "BANCAJ_CAJAS_GENPROC");
        	  getSesion(request).setID_Mensaje(idmensaje, mensaje);
        	  RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"BANCAJ_CAJAS_GENPROC","MCAJ||||",mensaje);
        	  irApag("/forsetiweb/caja_mensajes.jsp", request, response);
        	  return;
          }
          ...
          if(request.getParameter("ID") != null)
          {
            ...
            if (valoresParam.length == 1)
            {
              if(request.getParameter("subproceso") != null && request.getParameter("subproceso").equals("IMPRESION"))
              {
                ...
                String SQLCab = "select * FROM view_cajas_movimientos_impcab where ID = '" + p(request.getParameter("ID")) + "'";
              ...
              {
                  request.setAttribute("impresion", "CEFMovCajaDlg");
                  request.setAttribute("tipo_imp", "BANCAJ_CAJAS");
      ...
      throws ServletException, IOException
    {
		...
		HttpSession ses = request.getSession(true);
		JMovBancariosSes rec = (JMovBancariosSes)ses.getAttribute("mov_cajas_dlg");
        ...
        if(request.getParameter("bancaj").indexOf("FSI_CAJ_") != -1 &&
        		bancaj.equals(getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial()) )
        {
        ...
        if(rec.getMetPagoPol().equals("03"))
        {
        ...
        if(!rec.getFijo() && rec.getFijoDEST()) //si el origen es caja contable y el destino es banco o caja por fuera
        {
        ...
        else if(rec.getFijo() && !rec.getFijoDEST()) // Si el origen es un banco fijo y el destino es un banco o caja 
        {
        	...
        	if(rec.getMetPagoPol().equals("02") && (rec.getCuentaBanco().equals("") || (rec.getID_SatBanco().equals("000") && rec.getBancoExt().equals(""))))
            {
      ...
      throws ServletException, IOException
    {
		...
		if(rec.getMetPagoPol().equals("03") /*transferencia*/)
        {
    		idmensaje = 1; mensaje.append(JUtil.Msj("CEF", "BANCAJ_BANCAJ", "DLG", "MSJ-PROCERR4", 1)); //PRECAUCION: No se puede agregar un depósito o retiro de caja por un método de transferencia.
    		getSesion(request).setID_Mensaje(idmensaje, mensaje.toString());
    		return false;
        ...
        if(!rec.getFijo())
        {
        	...
        	if(rec.getMetPagoPol().equals("02") && (rec.getCuentaBanco().equals("") || (rec.getID_SatBanco().equals("000") && rec.getBancoExt().equals(""))))
            {
              idmensaje = 3; mensaje.append(JUtil.Msj("CEF", "BANCAJ_BANCAJ", "DLG", "MSJ-PROCERR4", 2)); //ERROR: Debes proporcionar numero, cuenta y banco del cheque para soporte de contabilidad electrónica
              getSesion(request).setID_Mensaje(idmensaje, mensaje.toString());
              return false;
            }
      ...
      throws ServletException, IOException
    {
    	...
    	if(proc.equals("APLICAR"))
    		...
    		str += "sp_bancos_movs_cierremes( '1','" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() +
        ...
        RDP("CEF", getSesion(request).getConBD(),(rfb.getIdmensaje() == 0 ? "OK" : (rfb.getIdmensaje() == 4 ? "AL" : "ER")), getSesion(request).getID_Usuario(), "BANCAJ_CAJAS_GENPROC", "MCAJ|" + rfb.getClaveret() + "|" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "||",rfb.getRes());
        irApag("/forsetiweb/caja_mensajes.jsp", request, response);
      ...
      throws ServletException, IOException
    {
        ...
        RDP("CEF", getSesion(request).getConBD(),(rfb.getIdmensaje() == 0 ? "OK" : (rfb.getIdmensaje() == 4 ? "AL" : "ER")), getSesion(request).getID_Usuario(), "BANCAJ_CAJAS_CANCELAR", "MCAJ|" + rfb.getClaveret() + "|" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "||",rfb.getRes());
        irApag("/forsetiweb/caja_mensajes.jsp", request, response);
      ...
      throws ServletException, IOException
    {
        ...
        str = "select * from  sp_bancos_trans_agregar( '1','" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "','" + p(JUtil.obtFechaSQL(pol.getFecha())) + "','" + p(pol.getConcepto()) +
            "','" + beneficiario + "','" + retiro + "','" + p(tipomov) + "','" +
        ...
        RDP("CEF",getSesion(request).getConBD(),(rfb.getIdmensaje() == 0 ? "OK" : (rfb.getIdmensaje() == 4 ? "AL" : "ER")),getSesion(request).getID_Usuario(), "BANCAJ_CAJAS_TRASPASAR", "MCAJ|" + rfb.getClaveret() + "|" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "||",rfb.getRes());
        irApag("/forsetiweb/caja_y_bancos/mov_bancaj_dlg_trsp.jsp", request, response);
      ...
      throws ServletException, IOException
    {
      ...
      str = "select * from  sp_bancos_movs_agregar( '1','" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "','" + p(JUtil.obtFechaSQL(pol.getFecha())) + "','" + p(pol.getConcepto()) +
      ...
      RDP("CEF",getSesion(request).getConBD(),(rfb.getIdmensaje() == 0 ? "OK" : (rfb.getIdmensaje() == 4 ? "AL" : "ER")),getSesion(request).getID_Usuario(), "BANCAJ_CAJAS_AGREGAR", "MCAJ|" + rfb.getClaveret() + "|" + getSesion(request).getSesion("BANCAJ_CAJAS").getEspecial() + "||",rfb.getRes());
      irApag("/forsetiweb/caja_y_bancos/mov_bancaj_dlg.jsp", request, response);
