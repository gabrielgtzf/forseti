...
public class JCierresCajaCtrl extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      String cierres_caja = "";
      request.setAttribute("cierres_caja",cierres_caja);
      ...
      if(getSesion(request).getEst("BANCAJ_CIERRES") == false)
      {
        ...
        String Entidad = "ID_Entidad = '" + set.getAbsRow(0).getID() + "'";
        ...
        getSesion(request).EstablecerCEF(request, "bancaj_cierres.png", "BANCAJ_CIERRES");
        getSesion(request).getSesion("BANCAJ_CIERRES").setParametros(Entidad, Tiempo, "", set.getAbsRow(0).getCuenta(), JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3), "");
        getSesion(request).getSesion("BANCAJ_CIERRES").setOrden(p(request.getParameter("etq")),"");
        getSesion(request).getSesion("BANCAJ_CIERRES").setEspecial(Integer.toString(set.getAbsRow(0).getID()));
            
        getSesion(request).setID_Mensaje(idmensaje, mensaje);
        RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"BANCAJ_CIERRES","CCAJ||" + set.getAbsRow(0).getID() + "||","");
        irApag("/forsetiweb/caja_y_bancos/cierres_caja_vsta.jsp",request,response);
        return;
      }
      ////////////////////////////////////////////////////////////
      if(request.getParameter("entidad") != null && !request.getParameter("entidad").equals(""))
      {
    	  ...
    	  if(set.getNumRows() > 0)
    	  {
    		  String Entidad = "ID_Entidad = '" + p(request.getParameter("entidad")) + "'";
    		  getSesion(request).getSesion("BANCAJ_CIERRES").setEntidad(Entidad,set.getAbsRow(0).getCuenta());
    		  getSesion(request).getSesion("BANCAJ_CIERRES").setEspecial(Integer.toString(set.getAbsRow(0).getID()));
    		  RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"BANCAJ_CIERRES","CCAJ||" + set.getAbsRow(0).getID() + "||","");
    	  }
    	  else
    	  {
    		  ...
    		  RDP("CEF",getSesion(request).getConBD(),"IA",getSesion(request).getID_Usuario(),"BANCAJ_CIERRES","CCAJ||" + p(request.getParameter("entidad")) + "||",mensaje);
    	  }
      }
      ...
      if(request.getParameter("tiempo") != null && !request.getParameter("tiempo").equals(""))
      {
    	  if(request.getParameter("tiempo").equals("HOY"))
    	  {
    		  Calendar fecha = GregorianCalendar.getInstance();
    		  String Tiempo = "date_part('day',Fecha) = " + JUtil.obtDia(fecha) + "  AND date_part('month',Fecha) = " + JUtil.obtMes(fecha) + " AND date_part('year',Fecha) = " + JUtil.obtAno(fecha);
    		  getSesion(request).getSesion("BANCAJ_CIERRES").setTiempo(Tiempo, JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("SEM"))
    	  {
    		  ...
    		  getSesion(request).getSesion("BANCAJ_CIERRES").setTiempo(Tiempo,JUtil.Msj("GLB", "GLB", "GLB", "SEMANA", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("MAS"))
    	  {
    		  if(request.getParameter("mes") != null && request.getParameter("ano") != null && !request.getParameter("mes").equals("") && !request.getParameter("ano").equals(""))
    		  {
    			  ...
    			  getSesion(request).getSesion("BANCAJ_CIERRES").setTiempo(Tiempo,TiempoTit);
    		  }
    		  else
    		  {
      ...
      {
    	  getSesion(request).getSesion("BANCAJ_CIERRES").setOrden(p(request.getParameter("etq")),p(request.getParameter("orden")));
      }
