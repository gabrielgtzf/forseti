...
public class JCompDevSes  extends JSesionRegs
{
	  ...
	  public JCompDevSes(HttpServletRequest request, String ID_Entidad, String usuario)
	  {
		  JComprasEntidadesSetIdsV2 set = new JComprasEntidadesSetIdsV2(request);
	  ...
	  public JCompDevSesPart getPartida(int ind)
	  {
	    return (JCompDevSesPart)m_Partidas.elementAt(ind);
	  }
	  ...
	  {
		  short res = -1;
		  JCompDevSesPart part = (JCompDevSesPart) m_Partidas.elementAt(indPartida);
		  ...
		  else
		  {
		     ...
		     res = 3;
		     mensaje.append("ERROR: La cantidad " + cantidad + " o el precio " + precio + " del producto especificado son mayores al de la recepción original " + part.getCantidadFact() + " y " + part.getPrecioFact() + ". No se puede cambiar la partida de la devolución");
		  }
  ...
  public void agregaPartida(float cantidad, String unidad, String idprod, String descripcion, float precio, float importe, float iva, String obs_partida, String idtipo) 
	  {
		    // Aqui aplica la partida
			JCompDevSesPart part = new JCompDevSesPart(cantidad, unidad, idprod, descripcion, precio, importe, 
	  ...
	  public boolean existeEnLista()
	  {
		...
		for(int i = 0; i < m_Partidas.size(); i++)
		{
		  	JCompDevSesPart part = (JCompDevSesPart) m_Partidas.elementAt(i);
	  	...
	  	throws ServletException, IOException
	  {
	      ...
	      identidad = Byte.parseByte(JUtil.getSesion(request).getSesionCompRecep().getEspecial());
	  	  ...
	  	  if(m_ID_Entidad != identidad)
		  {
	  	  ...
	  	  m_DevNum = Integer.parseInt(request.getParameter("devolucion"));
		  m_Referencia = request.getParameter("referencia");
		  m_Obs = request.getParameter("obs");
		  m_Fecha = JUtil.estFecha(request.getParameter("fecha")); 
