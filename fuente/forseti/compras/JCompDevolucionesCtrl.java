...
public class JCompDevolucionesCtrl extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      String comp_dev = "";
      request.setAttribute("comp_dev",comp_dev);

      String mensaje = ""; short idmensaje = -1;
      String usuario = getSesion(request).getID_Usuario();

      if(!getSesion(request).getPermiso("COMP_DEV"))
      {
          idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "COMP_DEV");
          getSesion(request).setID_Mensaje(idmensaje, mensaje);
          RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"COMP_DEV","CDEV||||",mensaje);
          irApag("/forsetiweb/caja_mensajes_vsta.jsp", request, response);
          return;
      }

      // establece en la sesion que los mensajes se estan configurando por primera ocasion
      if(getSesion(request).getEst("COMP_DEV") == false)
      {
        ...
        getSesion(request).EstablecerCEF(request, "comp_dev.png", "COMP_DEV");
        getSesion(request).getSesion("COMP_DEV").setParametros(Entidad, Tiempo, "", set.getAbsRow(0).getDescripcion(), JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3), JUtil.Elm(JUtil.Msj("CEF", "COMP_DEV", "VISTA", "STATUS"),1));
        getSesion(request).getSesion("COMP_DEV").setOrden(p(request.getParameter("etq")),"");
        getSesion(request).getSesion("COMP_DEV").setEspecial(Integer.toString(set.getAbsRow(0).getID_Entidad()));
  	  
        getSesion(request).setID_Mensaje(idmensaje, mensaje);
        RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"COMP_DEV","CDEV||" + set.getAbsRow(0).getID_Entidad() + "||","");
        irApag("/forsetiweb/compras/comp_dev_vsta.jsp",request,response);
        return;
      }
      ...
      if(request.getParameter("entidad") != null && !request.getParameter("entidad").equals(""))
      {
          ...
          {
          	String Entidad = "ID_Entidad = '" + set.getAbsRow(0).getID_Entidad() + "'";
          	getSesion(request).getSesion("COMP_DEV").setEntidad(Entidad,set.getAbsRow(0).getDescripcion());
          	getSesion(request).getSesion("COMP_DEV").setEspecial(Integer.toString(set.getAbsRow(0).getID_Entidad()));
          }
          else
          {
          	idmensaje = 1; mensaje += JUtil.Msj("GLB", "GLB", "GLB", "ERROR-PARAM", 1); // 1 Error de entidad
          	RDP("CEF",getSesion(request).getConBD(),"IA",getSesion(request).getID_Usuario(),"COMP_DEV","CDEV||" + p(request.getParameter("entidad")) + "||",mensaje);
          }
      }
      else if(request.getParameter("tiempo") != null && !request.getParameter("tiempo").equals(""))
      {
    	  if(request.getParameter("tiempo").equals("HOY"))
    	  {
    		  Calendar fecha = GregorianCalendar.getInstance();
    		  String Tiempo = "date_part('day',Fecha) = " + JUtil.obtDia(fecha) + "  AND date_part('month',Fecha) = " + JUtil.obtMes(fecha) + " AND date_part('year',Fecha) = " + JUtil.obtAno(fecha);
    		  getSesion(request).getSesion("COMP_DEV").setTiempo(Tiempo, JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("SEM"))
    	  {
    		  ...
    		  getSesion(request).getSesion("COMP_DEV").setTiempo(Tiempo,JUtil.Msj("GLB", "GLB", "GLB", "SEMANA", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("MAS"))
    	  {
    		  if(request.getParameter("mes") != null && request.getParameter("ano") != null && !request.getParameter("mes").equals("") && !request.getParameter("ano").equals(""))
    		  {
    			  ...
    			  getSesion(request).getSesion("COMP_DEV").setTiempo(Tiempo,TiempoTit);
    		  }
    		  else
    		  {
      ...
      else if(request.getParameter("status") != null && !request.getParameter("status").equals(""))
      {
    	  ...
    	  if(request.getParameter("status").equals("TODOS"))
    		  getSesion(request).getSesion("COMP_DEV").setStatus("",JUtil.Elm(sts,1));
    	  else if(request.getParameter("status").equals("GENERADOS"))
    		  getSesion(request).getSesion("COMP_DEV").setStatus("Status = 'G'",JUtil.Elm(sts,2));
    	  else if(request.getParameter("status").equals("CANCELADOS"))
    		  getSesion(request).getSesion("COMP_DEV").setStatus("Status = 'C'",JUtil.Elm(sts,3));
    	  else if(request.getParameter("status").equals("EMITIDOS"))
    		  getSesion(request).getSesion("COMP_DEV").setStatus("Status = 'E'",JUtil.Elm(sts,4));
    	  else if(request.getParameter("status").equals("REVERTIDOS"))
    		  getSesion(request).getSesion("COMP_DEV").setStatus("Status = 'R'",JUtil.Elm(sts,5));
    	  else
    	  {
      ...
      {
       	  getSesion(request).getSesion("COMP_DEV").setOrden(p(request.getParameter("etq")),p(request.getParameter("orden")));
      }
      getSesion(request).setID_Mensaje(idmensaje, mensaje);
      irApag("/forsetiweb/compras/comp_dev_vsta.jsp", request, response);
