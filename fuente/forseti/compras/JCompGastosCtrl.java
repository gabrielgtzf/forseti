...
public class JCompGastosCtrl extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      String comp_gas = "";
      request.setAttribute("comp_gas",comp_gas);

      String mensaje = ""; short idmensaje = -1;
      String usuario = getSesion(request).getID_Usuario();

      if(!getSesion(request).getPermiso("COMP_GAS"))
      {
          idmensaje = 3; mensaje += MsjPermisoDenegado(request, "CEF", "COMP_GAS");
          getSesion(request).setID_Mensaje(idmensaje, mensaje);
          RDP("CEF",getSesion(request).getConBD(),"NA",getSesion(request).getID_Usuario(),"COMP_GAS","CGAS||||",mensaje);
          irApag("/forsetiweb/caja_mensajes_vsta.jsp", request, response);
          return;
      }

      // establece en la sesion que los mensajes se estan configurando por primera ocasion
      if(getSesion(request).getEst("COMP_GAS") == false)
      {
        JComprasEntidadesSetIdsV2 set = new JComprasEntidadesSetIdsV2(request,usuario,"CEF-1",2);
        set.Open();
        ...
        getSesion(request).setID_Mensaje(idmensaje, mensaje);
        RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"COMP_GAS","CGAS||" + set.getAbsRow(0).getID_Entidad() + "||","");
        irApag("/forsetiweb/compras/comp_gas_vsta.jsp",request,response);
        return;
      }

      if(request.getParameter("entidad") != null && !request.getParameter("entidad").equals(""))
      {
          JComprasEntidadesSetIdsV2 set = new JComprasEntidadesSetIdsV2(request,usuario,p(request.getParameter("entidad")),2);
          set.Open();
          if(set.getNumRows() > 0)
          {
          	String Entidad = "ID_Entidad = '" + set.getAbsRow(0).getID_Entidad() + "'";
          	getSesion(request).getSesion("COMP_GAS").setEntidad(Entidad,set.getAbsRow(0).getDescripcion());
          	getSesion(request).getSesion("COMP_GAS").setEspecial(Integer.toString(set.getAbsRow(0).getID_Entidad()));
          }
          else
          {
          	idmensaje = 1; mensaje += JUtil.Msj("GLB", "GLB", "GLB", "ERROR-PARAM", 1); // 1 Error de entidad
          	RDP("CEF",getSesion(request).getConBD(),"IA",getSesion(request).getID_Usuario(),"COMP_GAS","CGAS||" + p(request.getParameter("entidad")) + "||",mensaje);
          }
      }
      else if(request.getParameter("tiempo") != null && !request.getParameter("tiempo").equals(""))
      {
    	  if(request.getParameter("tiempo").equals("HOY"))
    	  {
    		  Calendar fecha = GregorianCalendar.getInstance();
    		  String Tiempo = "date_part('day',Fecha) = " + JUtil.obtDia(fecha) + "  AND date_part('month',Fecha) = " + JUtil.obtMes(fecha) + " AND date_part('year',Fecha) = " + JUtil.obtAno(fecha);
    		  getSesion(request).getSesion("COMP_GAS").setTiempo(Tiempo, JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("SEM"))
    	  {
    		  ...
    		  getSesion(request).getSesion("COMP_GAS").setTiempo(Tiempo,JUtil.Msj("GLB", "GLB", "GLB", "SEMANA", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("MAS"))
    	  {
    		  if(request.getParameter("mes") != null && request.getParameter("ano") != null && !request.getParameter("mes").equals("") && !request.getParameter("ano").equals(""))
    		  {
    			  ...
    			  getSesion(request).getSesion("COMP_GAS").setTiempo(Tiempo,TiempoTit);
    		  }
    		  else
    		  {
      ...
      else if(request.getParameter("status") != null && !request.getParameter("status").equals(""))
      {
    	  ...
    	  if(request.getParameter("status").equals("TODOS"))
    		  getSesion(request).getSesion("COMP_GAS").setStatus("",JUtil.Elm(sts,1));
    	  else if(request.getParameter("status").equals("GENERADOS"))
    		  getSesion(request).getSesion("COMP_GAS").setStatus("Status = 'G'",JUtil.Elm(sts,2));
    	  else if(request.getParameter("status").equals("CANCELADOS"))
    		  getSesion(request).getSesion("COMP_GAS").setStatus("Status = 'C'",JUtil.Elm(sts,3));
    	  else
    	  {
      ...
      {
       	  getSesion(request).getSesion("COMP_GAS").setOrden(p(request.getParameter("etq")),p(request.getParameter("orden")));
      }
      getSesion(request).setID_Mensaje(idmensaje, mensaje);
      irApag("/forsetiweb/compras/comp_gas_vsta.jsp", request, response);
