...
public class JCatGastosSes extends JSesionRegsObjs
{
  ...
  public void agregaBodega(int idbodega, String nombre, boolean manejo)
    {
	  
      JCatProdSesBodegas obj = new JCatProdSesBodegas(idbodega, nombre, (!manejo ? -1 : 0), (!manejo ? -1 : 0));
...
public short agregaPartida(HttpServletRequest request, String cuenta, float porcentaje, StringBuffer mensaje)
  {
    ...
    if(set.getNumRows() > 0)
    {
      ...
      {
        res = 1;
        mensaje.append(JUtil.Msj("CEF", "INVSERV_INVSERV", "DLG", "MSJ-PROCERR2", 3)); //"PRECAUCION: El porcentaje no puede ser menor a cero");
      }
      else
      {
  ...
  public short editaPartida(int indPartida, HttpServletRequest request, String cuenta, float porcentaje, StringBuffer mensaje)
  {
    ...
    if (set.getNumRows() > 0)
    {
      ...
      {
        res = 3;
        mensaje.append(JUtil.Msj("CEF", "INVSERV_INVSERV", "DLG", "MSJ-PROCERR2", 3)); //"PRECAUCION: Al editar, el porcentaje no puede ser menor que 0");
      }
      else
      {
        String nombre = set.getAbsRow(0).getNombre();
