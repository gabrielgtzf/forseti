...
public class JAlmTraspasosCtrl extends JForsetiApl
{
      ...
      throws ServletException, IOException
    {
      ...
      String alm_traspasos = "";
      request.setAttribute("alm_traspasos",alm_traspasos);
      ...
      if(getSesion(request).getEst("ALM_TRASPASOS") == false)
      {
        ...
        getSesion(request).EstablecerCEF(request, "alm_traspasos.png", "ALM_TRASPASOS");
        getSesion(request).getSesion("ALM_TRASPASOS").setParametros(Entidad, Tiempo, "", set.getAbsRow(0).getNombre(), JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3), JUtil.Elm(JUtil.Msj("CEF", "ALM_TRASPASOS", "VISTA", "STATUS"),1));
        getSesion(request).getSesion("ALM_TRASPASOS").setOrden(p(request.getParameter("etq")),"");
        getSesion(request).getSesion("ALM_TRASPASOS").setEspecial(Integer.toString(set.getAbsRow(0).getID_Bodega()));
       
        getSesion(request).setID_Mensaje(idmensaje, mensaje);
        RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"ALM_TRASPASOS","TALM||" + set.getAbsRow(0).getID_Bodega() + "||","");
        irApag("/forsetiweb/almacen/alm_traspasos_vsta.jsp",request,response);
        return;
      }
      ...
      if(request.getParameter("entidad") != null && !request.getParameter("entidad").equals(""))
      {
        ...
        {
        	String Entidad = "ID_Bodega = '" + set.getAbsRow(0).getID_Bodega() + "'";
        	getSesion(request).getSesion("ALM_TRASPASOS").setEntidad(Entidad,set.getAbsRow(0).getNombre());
        	getSesion(request).getSesion("ALM_TRASPASOS").setEspecial(Integer.toString(set.getAbsRow(0).getID_Bodega()));
        	RDP("CEF",getSesion(request).getConBD(),"OK",getSesion(request).getID_Usuario(),"ALM_TRASPASOS","TALM||" + set.getAbsRow(0).getID_Bodega() + "||","");
        ...
        else
        {
           ...
           RDP("CEF",getSesion(request).getConBD(),"IA",getSesion(request).getID_Usuario(),"ALM_TRASPASOS","TALM||" + p(request.getParameter("entidad")) + "||",mensaje);
        }
      ...
      if(request.getParameter("tiempo") != null && !request.getParameter("tiempo").equals(""))
      {
    	  if(request.getParameter("tiempo").equals("HOY"))
    	  {
    		  Calendar fecha = GregorianCalendar.getInstance();
    		  String Tiempo = "date_part('day',Fecha) = " + JUtil.obtDia(fecha) + "  AND date_part('month',Fecha) = " + JUtil.obtMes(fecha) + " AND date_part('year',Fecha) = " + JUtil.obtAno(fecha);
    		  getSesion(request).getSesion("ALM_TRASPASOS").setTiempo(Tiempo, JUtil.Msj("GLB", "GLB", "GLB", "HOY", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("SEM"))
    	  {
    		  ...
    		  getSesion(request).getSesion("ALM_TRASPASOS").setTiempo(Tiempo,JUtil.Msj("GLB", "GLB", "GLB", "SEMANA", 3));
    	  }
    	  else if(request.getParameter("tiempo").equals("MAS"))
    	  {
    		  if(request.getParameter("mes") != null && request.getParameter("ano") != null && !request.getParameter("mes").equals("") && !request.getParameter("ano").equals(""))
    		  {
    			  ...
    			  getSesion(request).getSesion("ALM_TRASPASOS").setTiempo(Tiempo,TiempoTit);
    		  }
    		  else
    		  {
      ...
      if(request.getParameter("status") != null && !request.getParameter("status").equals(""))
      {
    	  ...
    	  {
    		  getSesion(request).getSesion("ALM_TRASPASOS").setStatus("",JUtil.Elm(sts,1));
    	  }
    	  else if(request.getParameter("status").equals("GUARDADOS"))
    	  {
    		  getSesion(request).getSesion("ALM_TRASPASOS").setStatus("Status = 'G'",JUtil.Elm(sts,2));
    	  }
    	  else if(request.getParameter("status").equals("EMITIDOS"))
    	  {
    		  getSesion(request).getSesion("ALM_TRASPASOS").setStatus("Status = 'E'",JUtil.Elm(sts,3));
    	  }
    	  else if(request.getParameter("status").equals("REVERTIDOS"))
    	  {
    		  getSesion(request).getSesion("ALM_TRASPASOS").setStatus("Status = 'R'",JUtil.Elm(sts,4));
    	  }
    	  else if(request.getParameter("status").equals("PENDIENTES"))
    	  {
    		  getSesion(request).getSesion("ALM_TRASPASOS").setStatus("Status = 'P'",JUtil.Elm(sts,5));
    	  }
    	  else if(request.getParameter("status").equals("CANCELADOS"))
    	  {
    		  getSesion(request).getSesion("ALM_TRASPASOS").setStatus("Status = 'C'",JUtil.Elm(sts,6));
    	  }
    	  else
    	  {
      ...
      {
       	  getSesion(request).getSesion("ALM_TRASPASOS").setOrden(p(request.getParameter("etq")),p(request.getParameter("orden")));
      }
